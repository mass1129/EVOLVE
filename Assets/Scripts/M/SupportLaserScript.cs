...
using UnityEngine;
using Photon.Pun;
public class SupportLaserScript : MonoBehaviourPun {
    ...
    private float damageTicTime = 0.5f;
	[SerializeField] private LayerMask laserLayerMask = new LayerMask();
	...
	public void RpcUpdateLaser()
    {
		...
		if (cam != null) // endPoint가 캠일 때
		{
			...
			if (Physics.Raycast(rayMouse.origin, rayMouse.direction, out hit, maximumLenght, laserLayerMask))
			{
				if (hit.collider)
				{
					for (int i = 0; i < lineRenderers.Count; i++)
		...
		else // endPoint가 캠이 아닐 때
		{
			...
			if (Physics.Raycast(rayMouse.origin, rayMouse.direction, out hit, maximumLenght, laserLayerMask))
			{
				if (hit.collider)
				{
					...
					if (hit.collider.gameObject.GetComponent<HitBox>() == true) ///////////// 몬스터에게 데미지 주는 부분
					{
						currentTime += Time.deltaTime;
						if(currentTime > damageTicTime)
                        {
							var hitBox = hit.collider.GetComponent<HitBox>();
							if (hitBox)
							{
								Debug.Log("Laser hit");
								hitBox.OnLasertHit(20, transform.forward);
							}
							currentTime = 0;
						}
