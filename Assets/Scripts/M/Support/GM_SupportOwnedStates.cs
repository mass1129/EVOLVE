...
namespace GM_SupportOwnedStates
{
    ...
    public class Global : GM_State<GM_Hunter>
    {
        ...
        float barrierWaitTime = 40;         // 서포트 스킬 대기 시간
        float barrierCoolTime = 40;         // 서포트 스킬 쿨타임. 스킬 사용과 동시에 0으로 변하며, 바로 다시 증가.
        ...
        public override void Execute(GM_Hunter entity)
        {
            ...
            // Support 스킬 발동 (주변 방어막 충전)
            if (Input.GetKeyDown(KeyCode.Alpha4) && barrierCoolTime > barrierWaitTime && entity.photonView.IsMine)
            {
                entity.SupportBarrier();  // GM_Support 클래스에 해당 스킬에 대한 RPC 함수 존재
                barrierCoolTime = 0;
            }
            ...
            entity.Skill2Slider.maxValue = barrierWaitTime;
            entity.Skill2Slider.value = barrierCoolTime;
        }
    ...
    {
        SupportLaserScript laserScript;

        public override void Enter(GM_Hunter entity)
        {
            entity.SetTrigger("Attack");
            laserScript = entity.GetComponentInChildren<SupportLaserScript>();
            if (entity.weaponType == 0)
            {
                laserScript.EnableLaser();
                entity.WeaponSetTrigger("Spin");
            }
        ...
        public override void Execute(GM_Hunter entity)
        {
            ...
            if (entity.weaponType == 0)
            {
                ...
                if (Input.GetMouseButtonUp(0))
                {
                    ...
                    entity.WeaponSetTrigger("Inactive");
                }
            }
        ...
        {
            entity.ResetTrigger("Attack");
            if(laserScript && entity.weaponType == 0)
            {
                laserScript.DisableLaserCaller(laserScript.disableDelay);
            }
            entity.WeaponResetTrigger("Spin");
            entity.WeaponSetTrigger("Inactive");
        }
    }
