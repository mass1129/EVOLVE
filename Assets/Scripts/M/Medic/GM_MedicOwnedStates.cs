...
{   
    ...
    {
        // 각 클래스 선언
        GameObject assult = GameObject.Find("Assult");
    ...
    {
        float currentTime = 0;
        float accelMaxTime = 10;        // 가속 스킬 지속 시간
        float accelCoolTime = 20;       // 가속 스킬 쿨타임. 스킬 지속 시간 중에는 0으로 유지되고, 스킬 사용 중이 아닐 때 증가.
        float healWaitTime = 40;         // 메딕 스킬 대기 시간
        float healCoolTime = 40;        // 메딕 스킬 쿨타임. 스킬 사용과 동시에 0으로 변하며, 바로 다시 증가.
        ...
        public override void Execute(GM_Hunter entity)
        {
            ...
            // 가속 스킬 발동
            if(Input.GetKeyDown(KeyCode.Alpha3) && accelCoolTime > 20)
            {
                isAccelerated = true;
            }
            ...
            // Medic 스킬 발동 (주변 회복)
            if (Input.GetKeyDown(KeyCode.Alpha4) && healCoolTime > healWaitTime && entity.photonView.IsMine)
            {
                entity.Heal();  // GM_Medic 클래스에 해당 스킬에 대한 RPC 함수 존재
                healCoolTime = 0;
            }
            ...
            entity.Skill2Slider.maxValue = healWaitTime;
            entity.Skill2Slider.value = healCoolTime;
        }
    ...
    {
        MedicLaserScript laserScript;

        public override void Enter(GM_Hunter entity)
        {
            entity.SetTrigger("Attack");
            laserScript = entity.GetComponentInChildren<MedicLaserScript>();
