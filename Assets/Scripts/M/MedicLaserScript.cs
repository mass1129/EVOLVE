...
using UnityEngine;
using Photon.Pun;
public class MedicLaserScript : MonoBehaviourPun {
	...
	private float healTicTime = 0.5f;
		...
		else // endPoint가 캠이 아닐 때
		{
			...
			if (Physics.Raycast(rayMouse.origin, rayMouse.direction, out hit, maximumLenght))
			{
				if (hit.collider)
				{
					...
					if (hit.collider.gameObject.GetComponent<GM_Hunter>() == true) ///////////// 다른 헌터 체력 회복해주는 부분
					{
						currentTime += Time.deltaTime;
						if(currentTime > healTicTime)
                        {
							if(hit.collider.gameObject.GetComponent<GM_Hunter>().photonView.IsMine) // 맞은 사람이 Mine이면(Mine PC에서 다른 PC로 OnSeralizePhontonView로 전달하기 위함)
                            {
								hit.collider.gameObject.GetComponent<GM_Hunter>().Hp += 5;
								currentTime = 0;
							}
						}		
					}
