...
public class LaserScript : MonoBehaviourPun {
    ...
    void Start () {
        ...
        if (FindObjectOfType<CharacterMotion>() != null)
            monster = FindObjectOfType<CharacterMotion>().gameObject;
    }
	...
	public void EnableLaser (){
		photonView.RPC("RpcEnableLaser", RpcTarget.All);
	}

    [PunRPC]
	public void RpcEnableLaser()
    {
		for (int i = 0; i < lineRenderers.Count; i++)
	...
	public void UpdateLaser () {
		photonView.RPC("RpcUpdateLaser", RpcTarget.All);
	}

    [PunRPC]
	public void RpcUpdateLaser()
    {
		if (firePoint != null)
		{
			for (int i = 0; i < lineRenderers.Count; i++)
		...
		else // endPoint가 캠이 아닐 때
		{
			RaycastHit hit;
			rayMouse = new Ray(firePoint.transform.position, endPoint.transform.position - firePoint.transform.position);
			...
			if (Physics.Raycast(rayMouse.origin, rayMouse.direction, out hit, maximumLenght))
			{
				if (hit.collider)
				{
					...
					if (hit.collider.gameObject.GetComponent<CharacterMotion>()) ///////////// 몬스터 속도 낮추는 부분
					{
			...
			{
				if (photonView.IsMine)      ///////////// 레이저 쏘는 중에 몬스터 안 맞으면 몬스터 속도원상 복구
					ReturnMonsterSpeed();
	...
	public void ReduceMonsterSpeed()
    {
		photonView.RPC("RpcReduceMonsterSpeed", RpcTarget.All);
	}

	[PunRPC]
	public void RpcReduceMonsterSpeed()
    {
		if (monster != null)
			monster.GetComponent<CharacterMotion>().groundSpeed = 0.3f;  ///////////// 몬스터 속도 감소
	}

	public void ReturnMonsterSpeed()
    {
		photonView.RPC("RpcReturnMonsterSpeed", RpcTarget.All);
    }

    [PunRPC]
	public void RpcReturnMonsterSpeed()
    {
		if (monster != null)
        {
			monster.GetComponent<CharacterMotion>().groundSpeed = 1.2f;  ///////////// 몬스터 속도 원상 복구
		}
	}
	...
	public void DisableLaserCaller (float timer){
		photonView.RPC("RpcDisableLaserCaller", RpcTarget.All, timer);
	}

    [PunRPC]
	public void RpcDisableLaserCaller(float timer)
	{
	...
	IEnumerator DisableLaser (float timer){
		...
		if (isStarting == false) {
			ReturnMonsterSpeed();				 ///////////// 레이저 발사 끝나면 몬스터 속도원상 복구
			isOver = false;
			for (int i = 0; i < lineRenderers.Count; i++) 
				lineRenderers[i].enabled = false;
		}
